...
! Printer et Array med n elementer.
  ...
  SUBROUTINE linSpace(array, n, startValue, stopValue)
	implicit none
	
	integer , intent(in) :: n;				!Number of elements
	real(wp) , intent(in) :: startValue;
	real(wp) , intent(in) :: stopValue;
	real(wp), intent(out) :: array(0:n);
	
    integer  :: i;
    real(wp) :: d;
    
    d = (stopValue - startValue)/n; 
...
implicit none
	...
	real(wp)				:: V_original(0:N); !# of voltages = N+1
	...
	integer, parameter 			:: out_unit=20
 	CHARACTER (LEN=*), PARAMETER 	:: filenameJ = 'resultsJ.txt';
	CHARACTER (LEN=*), PARAMETER 	:: filenameGS = 'resultsGS.txt';
	CHARACTER (LEN=*), PARAMETER 	:: filenameLU = 'resultsLU.txt';
	...
	call linSpace(V_original,N,V_N,V_0);
	V_original(N) = 0;
	V = V_original;
	!call printList(V_original,N);
	...
	open (unit=out_unit,file="resultsJ.txt",action="write", position = 'append')
	...
	open (unit=out_unit,file="resultsGS.txt",action="write", position = 'append')
	...
	open (unit=out_unit,file="resultsLU.txt",action="write", position = 'append')
