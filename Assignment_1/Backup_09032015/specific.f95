...
MODULE specific_functions
	...
	FUNCTION F(x,t, ww) result(Q)
		...
		!Pga potensialet ikke symmetrisk om origo. Plusser på perioden for å få xt positiv. (V kun definert for positive xt.)
		if(xt < 0) then
			xt = xt+1;
		endif
		tt = mod(t,tau*ww)
	...
	FUNCTION updatePos(x_1,t, ww) result(x_2)
		real(wp)		:: ww;
		real(wp)		:: x_1;
		real(wp)		:: x_2;
		real(wp)		:: t;
		
		x_2 = x_1 + F(x_1,t, ww)*ddt + sqrt(2*kbT*ddt/dU) * random_normal();
