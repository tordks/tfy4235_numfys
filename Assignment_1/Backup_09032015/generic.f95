...
SUBROUTINE init_random_seed()
	...
	!Number of elements.
	!Array elements.
	SUBROUTINE toFileArray(array, n, filename)
	...
	END  SUBROUTINE toFileArray
	
	!------------------------------------
	!toFile
	!------------------------------------

	SUBROUTINE toFile(matrix, Nr, Nc, filename, p)
		character(LEN=*), intent(in)	:: filename	
		integer , intent(in) 	:: p;!measurement period
		integer , intent(in) 	:: Nr;
		integer , intent(in) 	:: Nc;	
		real(wp), dimension(0:Nr-1, 0:Nc-1), intent(in)	:: matrix;
		integer 			 	:: i;	
		integer, parameter 		:: out_unit=1
	
		open (unit=out_unit,form="formatted", file=filename,action="write",status="replace")

		do i = 0, Nr-1
			write(out_unit,*) matrix(i,0:Nc-1:p)	!Skriver inn en og en kollonne inn i en linje.
		enddo
